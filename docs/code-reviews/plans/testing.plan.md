...
- The runner runs in-file tests concurrently by default. **New specs are not `.sequential` unless they have a documented reason to be** — concurrency-by-default is the runner posture recorded in [ADR-0005](../../adr/0005-test-runner-worker-model.md), and the `.sequential` carve-out is available but not the policy. A `.sequential` modifier appears in the spec only if its presence is justified at the call site (inline comment, ticket reference, or convention note).
