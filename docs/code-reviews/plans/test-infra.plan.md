...
- The probe diffs four state surfaces at each boundary: `globalThis` keys, `process` event listeners, active resources (`process.getActiveResourcesInfo()`), and fake-timer state (`vi.isFakeTimers()` OR `vi.getMockedSystemTime() !== null` — `setSystemTime` mocks the global `Date` without flipping `isFakeTimers()`, so both signals are necessary).
...
The probe's stderr output has three line shapes, each with a documented meaning. The shapes are the **contract** with anyone reading the log — changing a shape silently breaks log scrapers, CI grep rules, and human pattern-matching.
...
- `[LEAK <spec> > <test>] …` — state changed between the test's start and finish. Per-test scope.
- `[LEAK <spec> <file-exit>] …` — state changed across the whole file lifetime. Per-file scope.
