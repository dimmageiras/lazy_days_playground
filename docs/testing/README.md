...
The mode→env mechanism is load-bearing on three assumptions: the root config pass runs first, in the **same process** as the per-project passes, and `--mode=debug` reaches that root pass. Because `extends: true` re-evaluates the config per project with the default mode, the flag is persisted on `process.env` during the root pass and read back on every pass. A config-time guard throws when `--mode=debug` is present in `process.argv` but the flag does not resolve to `"1"`, so a broken round-trip fails loudly rather than running a vacuous green probe. Reverify these three assumptions — and that the guard still fires — on Vitest major bumps.
