...
The thin layer that **wires the runtime together**: creates the Fastify instance, hands it to the bootstrap factory, declares the application's own routes, registers the shutdown handoff plugin, and triggers the port claim. This area owns the *composition*, not the *behaviour* — the bootstrap module owns the latter.
...
- The entry contains *wiring only* — no business logic, no helpers defined inline
...
- Variable names match the concepts: `app`, `claimPort`, `shutdownRouteWithListeners` (or whatever the factory returns)
