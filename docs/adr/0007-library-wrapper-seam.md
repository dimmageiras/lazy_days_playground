...
- The constant's value list is maintained by hand; the mapped-type annotation guarantees it cannot be _wrong_ relative to the library, but a new library code still requires a human to add the entry (the compiler says exactly when and what).
