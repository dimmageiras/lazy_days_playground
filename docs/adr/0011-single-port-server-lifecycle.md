...
- [`./0017-environment-contract-extensions.md`](./0017-environment-contract-extensions.md) — partially supersedes the no-bind-address facet recorded here: a configurable bind address is now a validated, branded environment field. The single-listener, single-port, plain-HTTP, and build-vs-listen facets remain in force.
