...
- A **partial** supersession — a later ADR overturns only one facet of a multi-facet decision — keeps the original ADR's status `Accepted` (its still-in-force facets remain in force) and is recorded **only** through reciprocal `Related` links that name the superseded facet on both sides — never by flipping the whole ADR's status, and never by editing the superseded ADR's reasoning body, which stays a faithful record of the decision as accepted. Reserve the `Superseded by NNNN-...` status for a full supersession.
