...
- **Mutation is visible at the call site.** A helper whose contract is in-place mutation of an argument carries an `<verb>InPlace` suffix on the verb part of its name (`ExampleHelper.someActionInPlace(x)`), so a reader scanning a namespace can tell at a glance which entries mutate their inputs and which return fresh values.
...
- **The pattern extends to test-infrastructure helpers.** The same shape is used in the test-infrastructure helpers tree, with one additional contract from the runner ADR — every helper is a _stateless dispatcher_, holding no state at module scope. The namespace pattern does not by itself enforce statelessness; the runner contract does.
