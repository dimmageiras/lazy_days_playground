...
pnpm has built-in support for running multiple `package.json` scripts at once — no need to add an external parallel runner for common cases. This doc summarises the flags, the regex-pattern feature, the pnpm v11 behaviours that affect this workflow, and this project's orchestrator-selection guidance. Sections describing flags/behaviour are reference; sections under [When to reach for an external orchestrator](#when-to-reach-for-an-external-orchestrator) and the [Decision matrix](#decision-matrix) encode opinions, not pnpm's intrinsic behaviour.
...
pnpm's documentation pairs `--parallel` with `--recursive` for monorepos; in a single-package project, combining `--parallel` with the regex-selector form picks up the same concurrent-spawn behaviour against the installed pnpm version at the time of writing. **This single-package use is undocumented on pnpm.io but is upstream-intentional and regression-tested — not best-effort.** The real caveat is ordering, not single-package support: don't use `--parallel` where one script's output feeds another (see below). The trade-off: pnpm's built-in orchestration avoids adding another dependency for a small concurrency need, at the cost of riding behaviour that could shift across pnpm majors. On every pnpm major bump, reverify that `pnpm --parallel run "/<regex>/"` still resolves the selector and spawns matched scripts concurrently in a single-package layout; if it stops, switch to an external parallel runner before the bump lands.
...
pnpm couples `--aggregate-output` to `--reporter=append-only` implicitly — `--aggregate-output` only takes effect when the `append-only` reporter is active, and pnpm will pick it when the flag is set. The coupling is internal behaviour rather than a documented contract; on every pnpm major bump, reverify that `--aggregate-output` alone still selects the `append-only` reporter, and pin the reporter explicitly if it stops. See <https://pnpm.io/cli/run> for the current flag reference.
...
`append-only` is the reporter pnpm names explicitly for parallel-run scenarios — useful when the default interleaving is too chaotic but full aggregation isn't wanted. Check `pnpm run --help` against the installed pnpm version before relying on any other reporter name.
...
pnpm v11 is pure ESM internally and drops Node 18-21. The project itself pins a higher floor in `package.json#engines.node`; treat that pin as the authoritative minimum, with pnpm's own floor only relevant when the project loosens the pin below it.
