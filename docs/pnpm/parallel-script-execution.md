...
pnpm couples `--aggregate-output` to `--reporter=append-only` implicitly — `--aggregate-output` only takes effect when the `append-only` reporter is active, and pnpm will pick it when the flag is set.
...
pnpm v11 is pure ESM internally and drops Node 18-21.
