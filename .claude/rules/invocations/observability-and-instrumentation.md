...
- The tactical log-call rules — the Pino level vocabulary (`trace`…`fatal`, plus `silent` as configuration-only), the context-object-first signature, `normalizeError`, the emoji prefixes, redaction-path discipline, and the flush-before-exit rule — live canonically in [`../../../docs/logging/README.md`](../../../docs/logging/README.md). The [`logging-best-practices`](../../skills/logging-best-practices/SKILL.md) skill is the generic upstream Pino reference: carry over its transferable parts (the level table, the context-object-first signature), but it does **not** encode this project's `silent`-level, `normalizeError`, emoji-prefix, redaction-path, or flush-before-exit conventions — those exist only in the project doc. Also apply the [`logging-best-practices`](./logging-best-practices.md) invocation rule. **Read the project doc first; where it and the skill diverge on logging, the project doc wins.**
