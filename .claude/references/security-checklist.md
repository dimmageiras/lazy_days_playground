...
**Supply-chain hygiene** (`pnpm audit` won't catch malicious packages):

- [ ] Lockfile committed; CI installs with `pnpm install --frozen-lockfile` (not `pnpm install`)
...
| 6   | Vulnerable Components     | `pnpm audit`, keep deps updated, minimal deps         |
