...
| `docs/ts/`           | JSON schemas for the TypeScript toolchain (reference-only; not auto-consumed today — currently: `tsconfig.schema.json`) |
...
- Documentation passes the rename test — concepts and decisions are described in language that survives a refactor; current file paths appear only as labelled operational hints.
